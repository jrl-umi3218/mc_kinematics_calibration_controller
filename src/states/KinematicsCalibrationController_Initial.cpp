...
void KinematicsCalibrationController_Initial::configure(
    const mc_rtc::Configuration& config) {}

void KinematicsCalibrationController_Initial::start(
  ...
  mc_rtc::log::info("Using robot {} for KinematicsCalibrationController",
                    robot_);
  ...
  for (const auto& hole : holes) {
    for (const auto& position : positions) {
      ...
      mc_rtc::log::info("Creating async logger for hole {} at path {}",
                        loggerName, loggerPath);
      auto sink =
          std::make_shared<spdlog::sinks::basic_file_sink_mt>(loggerPath, true);
      auto [loggerElem, success] = holeLogger_.emplace(
          loggerName, std::make_shared<spdlog::logger>(loggerName, sink));
  ...
  ctl.gui()->addElement(
      ...
      ComboInput(
          "Position", KinematicsCalibrationController_Initial::positions,
          [this]() { return position_; },
          [this](const std::string& position) { position_ = position; }),
      ComboInput(
          "Hole", KinematicsCalibrationController_Initial::holes,
          [this]() { return hole_; },
          [this](const std::string& hole) { hole_ = hole; }),
...
bool KinematicsCalibrationController_Initial::run(
    mc_control::fsm::Controller& ctl_) {
  auto& ctl = static_cast<KinematicsCalibrationController&>(ctl_);
  output("OK");
...
void KinematicsCalibrationController_Initial::teardown(
    mc_control::fsm::Controller& ctl_) {
  auto& ctl = static_cast<KinematicsCalibrationController&>(ctl_);
...
void KinematicsCalibrationController_Initial::addHoleData(
    mc_control::fsm::Controller& ctl) {
  mc_rtc::log::info("Adding data for hole {}", hole_);
  auto loggerName = fmt::format("{}/{}", position_, hole_);
  if (holeLogger_.find(loggerName) == holeLogger_.end()) {
    mc_rtc::log::error("Logger {} not found in holeLogger_", loggerName);
    return;
  }
  holeLogger_[loggerName]->info(
      "{}", mc_rtc::io::to_string(ctl.realRobot(robot_).encoderValues(), ","));
};

void KinematicsCalibrationController_Initial::asyncSave(bool show) {
  for (auto& [_, logger] : holeLogger_) {
    if (show) {
      mc_rtc::log::info("Requesting async logger flush {}", logger->name());
    }
    logger->flush();
  }
}

EXPORT_SINGLE_STATE("KinematicsCalibrationController_Initial",
                    KinematicsCalibrationController_Initial)
